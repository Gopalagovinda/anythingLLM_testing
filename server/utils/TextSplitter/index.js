...
class TextSplitter {
   ...
   * @param {string} [config.splitterMode = "recursive"] - The splitter mode to use. Options: "recursive", "markdown_header".
   */
  constructor(config = {}) {
    this.config = config;
  ...
  #setSplitter(config = {}) {
    ...
    if (config?.splitterMode === "markdown_header") {
      return new MarkdownHeaderSplitter(splitterArgs);
    }
    return new RecursiveSplitter(splitterArgs);
  }
...
// Wrapper for Langchain MarkdownTextSplitter - splits on markdown structure (headers, paragraphs, etc.)
class MarkdownHeaderSplitter {
  ...
  log(text, ...args) {
    console.log(`\x1b[35m[MarkdownHeaderSplitter]\x1b[0m ${text}`, ...args);
  }
