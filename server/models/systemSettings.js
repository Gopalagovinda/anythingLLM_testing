...
const SystemSettings = {
  ...
  validations: {
    ...
    text_splitter_mode: (update) => {
      try {
        const validModes = ["recursive", "markdown_header"];
        if (!validModes.includes(update))
          throw new Error(
            `Invalid splitter mode. Must be one of: ${validModes.join(", ")}`
          );
        const { purgeEntireVectorCache } = require("../utils/files");
        purgeEntireVectorCache();
        return update;
        ...
        console.error(
          `Failed to run validation function on text_splitter_mode`,
          e.message
        );
        return "recursive";
      }
